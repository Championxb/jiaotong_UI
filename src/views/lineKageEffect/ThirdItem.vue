<template>
    ...
    <div class="center">
        <div class="show-item">
            <p class="title">{{ safeCurrentItem.title }} </p>
            <img 
                class="side-img" 
                :src="safeCurrentItem.imglist[safeNowNum].imgUrl"
                @click="toggleFullScreen(safeCurrentItem.imglist[safeNowNum].imgUrl)">
            <p class="side-title">{{ safeCurrentItem.imglist[safeNowNum].imgTitle || "5" }}</p>
            <div class="toggle">
...
<script setup>
    ...
    const tagIndex = ref(props.thirdMenu)
    const nowNum = ref(props.thirdIndex)
    const currentItem = ref({})
    const initialLoad = ref(true)
    watchEffect(() => {
        if(initialLoad.value)
        {
            nowNum.value = props.thirdIndex
            initialLoad.value = false
        } else {
            nowNum.value = 0
        }
        currentItem.value = imgs.value[tagIndex.value] || {}
    });
    const safeCurrentItem = computed(() => currentItem.value || {});
    ...
    const selectMenuItem = (index) => {
        ...
        currentItem.value = items.items[tagIndex.value]
        nowNum.value = 0
    }
    // 切换全屏状态的函数
    ...
    const allNum = computed(() => {
        return safeCurrentItem.value.imglist ? safeCurrentItem.value.imglist.length : 0;
    })
    // 切换到前一张或后一张图片
