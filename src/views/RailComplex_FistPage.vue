<template>
  <ScaleScreen :width="1920" :height="1080">
    <div class="box">
      ...
      <div class="contents">
        <div
          ...
          :style="{ '--ydeg': `${ydeg}deg` }"
          v-if="currentSelectButtomIndex == -1"
        >
          ...
          <div class="text_wrap">
            <vue3-seamless-scroll
              class="scroll-wrap"
              :list="FIRST_STATION_TEXT[stationIndex].text"
              :hover="true"
              :limitScrollNum="5"
              :step="0.5"
            >
              <div
                class="ui-wrap"
                v-for="(item, index) in FIRST_STATION_TEXT[stationIndex].text"
                :key="index"
              >
          ...
          <template class="stations">
            <ul>
              <li
                ...
                :style="[
                  getStyle(index),
                  index === selectedStationIndex1 ? stationSelectStyle : {},
                ]"
                @click="addBG1(index + 3)"
              >
                <div
                  class="station_bg"
                  :style="getStationDivStyle(index === selectedStationIndex1)"
                >
              ...
              <li
                ...
                :key="'second-' + index"
                :style="[
                  getStyle(index + firstHalf.length + 1),
                  index === selectedStationIndex2 ? stationSelectStyle : {},
                ]"
                @click="addBG2(index)"
              >
                <div
                  class="station_bg"
                  :style="getStationDivStyle(index === selectedStationIndex2)"
                >
    ...
    <Foot
      @getSelectButtonIndex="getSelectButtonIndex"
      :buttonIndex="currentSelectButtomIndex"
    ></Foot>
  </ScaleScreen>
</template>
<script setup>
...
import {
  FIRST_STATION_DATA,
  FIRST_STATION_TEXT,
} from "@/utils/firstStationData.js";
...
const onMouseleave = () => [(ydeg.value = 0)];
...
const getSelectButtonIndex = (index) => {
  ...
  switch (index) {
    case 0:
      selectTypeLength.value =
        FIRST_STATION_DATA[stationIndex.value].imgSrc.layout.length;
    ...
    case 1:
      selectTypeLength.value =
        FIRST_STATION_DATA[stationIndex.value].imgSrc.transfer.length;
      currentTypeAllImg.value = getImgSrc(
        selectTypeLength.value - 1,
        "transfer"
      );
      // console.log(currentTypeAllImg.value)
      shutDownInterval();
    ...
    case 2:
      selectTypeLength.value =
        FIRST_STATION_DATA[stationIndex.value].imgSrc.model.length;
    ...
    case 3:
      selectTypeLength.value =
        FIRST_STATION_DATA[stationIndex.value].imgSrc.facility.length;
      currentTypeAllImg.value = getImgSrc(
        selectTypeLength.value - 1,
        "facility"
      );
...
const firstHalf = computed(() =>
  currentStations.value.slice(0, Math.ceil(currentStations.value.length / 2))
); //slice() 方法可从已有的数组中返回选定的元素。
const secondHalf = computed(() =>
  currentStations.value.slice(Math.ceil(currentStations.value.length / 2))
);
    ...
    .content_right {
      ...
      .right_img {
        ...
        .img_wrap:nth-child(1) {
          img {
            transform: perspective(1600px) rotate(1deg) rotateX(10deg)
              rotateY(-22deg);
          }
        }
        ...
        .img_wrap:nth-child(2) {
          img {
            transform: perspective(500px) rotate(1deg) rotateX(0deg)
              rotateY(-10deg);
          }
